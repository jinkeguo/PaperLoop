	...
	describe('In the top frame', function() {
		...
		describe("Saving", function() {
			...
			describe("To Zotero", function() {
				...
				it('includes a PaperLoop thought as a child-note payload', async function () {
					let restoreStub = await stubConnectorCallMethod({
						saveItems: { returnPayload: true },
						getSelectedCollection: { response: {} }
					});
					const note = '<h1>PaperLoop 思考</h1><p>用于讨论部分。</p>';
					try {
						var items = await background(async function(tabId, note) {
							let tab = await browser.tabs.get(tabId);
							return Zotero.Connector_Browser.saveWithTranslator(tab, 0, {note});
						}, tab.tabId, note);
					}
					finally {
						await restoreStub();
					...
					assert.equal(items.length, 1);
					assert.isArray(items[0].notes);
					assert.equal(items[0].notes.length, 1);
					assert.equal(items[0].notes[0].note, note);
				});
				...
				it('saves as snapshot', async function () {
					...
					try {
						...
						var frame = await tab.page.waitForFrame(frame => frame.url().startsWith(frameURL));
						var elem = await frame.waitForSelector('.ProgressWindow-progressBox');
						const expectedTitle = "Scarcity or Abundance? Preserving the Past in a Digital Era";
						await frame.waitForFunction(
							title => document.querySelector('.ProgressWindow-progressBox')
								?.textContent.includes(title),
							{},
							expectedTitle
						);
						var message = await elem.evaluate(node => node.textContent);
						assert.include(message, expectedTitle);
					} finally {
						await restoreStub();
					}
				});
			...
			describe("To zotero.org", function() {
				...
				it('displays a prompt when attempting to save to zotero.org for the first time', async function () {
					try {
						...
						var frame = await tab.page.waitForFrame(frame => frame.url().startsWith(frameURL));
						const expected = 'The Zotero Connector was unable to communicate with the Zotero desktop application.';
						await frame.waitForFunction(
							expected => document.querySelector('#zotero-modal-prompt')
