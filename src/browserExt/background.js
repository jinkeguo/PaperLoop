...
Zotero.Connector_Browser = new function() {
	...
	function _updateInfoForTab(tabId, url) {
		...
		const paperLoopPinned = !!tabInfo.paperLoopPinned;
	...
	let _paperLoopAutoOpen = true;

	async function _paperLoopLoadAutoOpen() {
		try {
			const stored = await browser.storage.local.get(PAPERLOOP_AUTO_OPEN_KEY);
			if (typeof stored[PAPERLOOP_AUTO_OPEN_KEY] === 'boolean') {
				_paperLoopAutoOpen = stored[PAPERLOOP_AUTO_OPEN_KEY];
			}
		}
		catch (e) {
			Zotero.logError(e);
		}
		return _paperLoopAutoOpen;
	}
	...
	async function _paperLoopMaybeAutoOpen(tab) {
		if (!_paperLoopAutoOpen || !tab || !tab.id) return {open: false};
		const tabInfo = Zotero.Connector_Browser.getTabInfo(tab.id);
		if (!tabInfo.translators || !tabInfo.translators.length) return {open: false};
		...
		return _paperLoopPublishState(tab, {open: true});
	}
	...
	async function _paperLoopSetActionError(tab, message=null) {
		try {
			await browser.action.setBadgeText({tabId: tab.id, text: message ? '!' : ''});
			if (message) {
	...
	this.paperLoopSetAutoOpen = async function(enabled, tab, frameId) {
		if (!tab || frameId !== 0) return {enabled: _paperLoopAutoOpen};
		_paperLoopAutoOpen = !!enabled;
		await browser.storage.local.set({[PAPERLOOP_AUTO_OPEN_KEY]: _paperLoopAutoOpen});
		return {enabled: _paperLoopAutoOpen};
	};
	...
	this.paperLoopAppendThought = async function(payload={}, tab, frameId) {
		...
		if (!note) throw new Error('请输入要同步的思考');
	...
	this.paperLoopSaveThought = async function(payload={}, tab, frameId) {
		...
		const savePromise = (async () => {
			...
			if (existingLink && tabInfo.translators && tabInfo.translators.length) {
				try {
