...
Zotero.UI.ModalPrompt = class ModalPrompt extends React.Component {
	...
	render() {
		...
		if (this.props.input) {
			let inputControl = this.props.inputMultiline
				? (<textarea
		...
		const dockedClass = this.props.docked ? " docked" : "";
		return (<div className={`ModalPrompt-overlay${dockedClass}`} onClick={onClickOutside}>
			<div className={`ModalPrompt${dockedClass}`} role="dialog" aria-labelledby="zotero-modal-prompt-title"
...
Zotero.UI.ModalPrompt.PropTypes = {
	...
	/**
	 * Render as a right-side panel so the underlying page remains readable.
	 */
