var PaperLoopDOIBridge = {
	...
	register() {
		Zotero.Server.Endpoints[this.endpointPath] = this.Endpoint;
		Zotero.Server.Endpoints[this.appendEndpointPath] = this.AppendNoteEndpoint;
		Zotero.Server.Endpoints[this.searchEndpointPath] = this.SearchItemsEndpoint;
		Zotero.Server.Endpoints[this.stateEndpointPath] = this.StateEndpoint;
		Zotero.Server.Endpoints[this.addPDFEndpointPath] = this.AddPDFEndpoint;
	},

	unregister() {
		delete Zotero.Server.Endpoints[this.endpointPath];
		delete Zotero.Server.Endpoints[this.appendEndpointPath];
		delete Zotero.Server.Endpoints[this.searchEndpointPath];
		delete Zotero.Server.Endpoints[this.stateEndpointPath];
		delete Zotero.Server.Endpoints[this.addPDFEndpointPath];
	},
...
PaperLoopDOIBridge.Endpoint.prototype = {
	...
	async init(requestData) {
		...
		for (const incoming of data.items || []) {
			...
			const matches = await PaperLoopDOIBridge.findExisting(library.libraryID, doi);
			if (!matches.length) {
