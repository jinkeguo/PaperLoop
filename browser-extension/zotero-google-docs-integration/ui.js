...
(function () {
  ...
  Zotero.GoogleDocs.UI = {
    ...
    displayCorruptCitationsAlert: async function (citations) {
      ...
      await this.activate();
      const citationList = citations.map(citation => `"${Zotero.Utilities.htmlSpecialChars(citation.text)}"`).join('<br/>');
      let result = await Zotero.Inject.confirm({
        title: Zotero.getString('general_warning'),
        button2Text: "",
        message: Zotero.getString('integration_googleDocs_corruptCitations_alert', ZOTERO_CONFIG.CLIENT_NAME) + '<br/><br/>' + citationList
      });
    ...
    selectText: async function (text, url = null) {
      ...
      try {
        ...
        await Zotero.Promise.delay();
        var openFindAndReplaceDialogKey = {
          ctrlKey: true,
          key: 'h',
          keyCode: 72
        };
        if (Zotero.isMac) {
          openFindAndReplaceDialogKey = {
            metaKey: true,
            key: 'h',
            keyCode: 72
          };
        ...
        findInput.dispatchEvent(new KeyboardEvent('input', {
          bubbles: true
        }));
