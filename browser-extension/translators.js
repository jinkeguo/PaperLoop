...
Zotero.Translators = new function() {
	...
	this._doUpdateFromRemote = async function(reset) {
		...
		Zotero.debug('Retrieving translators from Zotero Client for merged catalog');
		try {
			let translatorMetadata = await Zotero.Repo.getTranslatorMetadataFromZotero();
			await this.loadNewMetadata(translatorMetadata, reset, false);
			localLoaded = true;
		}
		catch (e) {
			localError = e;
			Zotero.debug('Failed to retrieve translators from Zotero Client; continuing with Repo');
		}

		try {
			Zotero.debug('Retrieving translators from Zotero Repo for merged catalog');
			let translatorMetadata = await Zotero.Repo.getTranslatorMetadataFromServer(reset);
			// A local reset has already established the base catalog. Merge the full
			// Repo response without deleting local-only/community translators.
			await this.loadNewMetadata(translatorMetadata, reset && !localLoaded, true);
			return;
		}
		catch (e) {
			repoError = e;
			if (localLoaded) {
				Zotero.debug('Failed to retrieve translators from Zotero Repo; retaining Zotero Client catalog');
				return;
			}
		}

		Zotero.logError('Failed to retrieve translators from Zotero Client and Zotero Repo '
			+ (repoError || localError));
		throw repoError || localError;
	}
	...
	this.loadNewMetadata = async function(newMetadata, reset=false, fromRepo=false) {
		...
		updatedTranslators = updatedTranslators.filter((translatorMetadata) => {
			...
			// Merge Zotero Client and Repo metadata without downgrading newer local
			// community translators such as CNKI and Wanfang Data.
			if (translator.lastUpdated === translatorMetadata.lastUpdated) {
				return false;
			}
			if (Zotero.Date.sqlToDate(translatorMetadata.lastUpdated)
					<= Zotero.Date.sqlToDate(translator.lastUpdated)) {
				Zotero.debug(`Translators: Keeping newer local ${translator.label}`);
				return false;
			}
