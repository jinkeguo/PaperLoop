...
let ItemSaver = function(options) {
	...
	this._paperLoop = options.paperLoop || null;
	this._items = [];
...
ItemSaver.prototype = {
	...
	_saveToZotero: async function (items, attachmentCallback, itemsDoneCallback=()=>0) {
		...
		const attachPaperLoopResults = (results, defaultLibraryID=null) => {
			for (const result of results || []) {
				if (result.libraryID == null && defaultLibraryID != null) {
					result.libraryID = defaultLibraryID;
				}
				const item = allItems.find(candidate => candidate.id === result.id);
				if (item) item.paperLoop = result;
			}
		};
		const paperLoopRequest = (requestItems, sessionID) => ({
			eventID: this._paperLoop.eventID,
			sessionID,
			targetID: this._paperLoop.targetID || "",
			note: this._paperLoop.note || "",
		...
		if (this._paperLoop) {
			let resolution;
			try {
				resolution = await Zotero.Connector.callMethod(
					"paperloop/resolve",
					paperLoopRequest(items)
				);
			}
			catch (e) {
				e.message = "PaperLoop Zotero 插件未就绪，已停止收藏以避免生成重复条目。"
					+ (e.message ? ` ${e.message}` : "");
				throw e;
			}
			if (items.some(item => !String(item.DOI || '').trim()) && !resolution.sessionResolution) {
				throw new Error('请先安装并启用 PaperLoop for Zotero 0.5.4，再保存这篇没有 DOI 的文献');
			}
			attachPaperLoopResults(resolution.items, resolution.libraryID);
			await this._paperLoopSupplementExistingPDFs(
				allItems,
				resolution.items || [],
				attachmentCallback || (() => {})
			);
			...
			items = items.filter(item => !handledIDs.has(item.id));
			if (!items.length) {
				this._items = [];
				this._singleFile = false;
				this._snapshotAttachment = null;
				itemsDoneCallback(allItems);
				Zotero.debug("PaperLoop: Existing DOI matched; appended note without creating an item");
				return allItems;
			}

			// Items without a DOI cannot be re-resolved after saving. Keep the
			// PaperLoop note in the native item payload for that bounded fallback.
			for (const item of items) {
				if (!String(item.DOI || "").trim() && this._paperLoop.note) {
					if (!Array.isArray(item.notes)) item.notes = [];
					item.notes.push({note: this._paperLoop.note});
				}
			}
		}
		...
		if (this._paperLoop && this._paperLoop.targetID) {
			await Zotero.Connector.callMethod("updateSession", {
				sessionID: this._sessionID,
				target: this._paperLoop.targetID
			});
		}
		if (this._paperLoop) {
			// Resolve the actual Connector session, not just the DOI. Generic
			// metadata translators can legitimately return an item without a DOI.
			const resolvableItems = items;
			if (resolvableItems.length) {
				...
				for (let attempt = 0; attempt < 12; attempt++) {
					resolution = await Zotero.Connector.callMethod(
						"paperloop/resolve",
						paperLoopRequest(resolvableItems, this._sessionID)
					);
					const resultByID = new Map(
						(resolution.items || []).map(result => [result.id, result])
					);
					linked = resolvableItems.every(item => {
						const result = resultByID.get(item.id);
						return result && result.status === "existing" && result.itemKey
							&& (!this._paperLoop.note || result.noteKey);
					});
					if (linked) break;
					await Zotero.Promise.delay(Math.min(150 + attempt * 50, 500));
				}
				if (!linked) {
					throw new Error(
						"文献条目已保存，但 PaperLoop 未能确认思考笔记。"
						+ "请确认 PaperLoop for Zotero 0.5.4 已启用后再试。"
					);
				}
				attachPaperLoopResults(resolution.items, resolution.libraryID);
			}
	...
	async _paperLoopSupplementExistingPDFs(items, results, attachmentCallback) {
		const downloadAssociatedFiles = await Zotero.Connector.getPref('downloadAssociatedFiles');
		for (const result of results) {
			if (!result || result.status !== 'existing' || result.hasPDF !== false) continue;
			result.pdfMissing = true;
			if (!downloadAssociatedFiles) {
				result.pdfSkipped = 'DOWNLOAD_ASSOCIATED_FILES_DISABLED';
				continue;
			}
			if (result.filesEditable === false) {
				result.pdfSkipped = 'FILES_NOT_EDITABLE';
				continue;
			}
			const item = items.find(candidate => candidate.id === result.id);
			const candidates = (item && Array.isArray(item.attachments) ? item.attachments : [])
				.filter(attachment => attachment
					&& attachment.mimeType === 'application/pdf'
					&& attachment.snapshot !== false
					&& /^https?:\/\//i.test(String(attachment.url || ''))
				);
			const attachment = candidates.find(candidate => candidate.isPrimary) || candidates[0];
			if (!attachment) {
				result.pdfAvailable = false;
				continue;
			}
			result.pdfAvailable = true;
			result.pdfBackfillAttempted = true;
			const payload = {
				...attachment,
			...
			try {
				const response = await Zotero.ItemSaver.saveExistingPDFToZotero(payload);
				result.hasPDF = !!response.hasPDF;
				result.pdfMissing = !result.hasPDF;
				result.pdfAdded = !!response.created;
				result.pdfAlreadyPresent = !!response.alreadyPresent;
				result.pdfAttachmentKey = response.attachmentKey || null;
				attachmentCallback(attachment, 100);
			}
			catch (e) {
				result.pdfError = e && e.message ? e.message : 'PDF 补充失败';
