...
var Zotero = global.Zotero = new function() {
	...
	this.migrate = async function() {
		...
		if (Zotero.isChromium) {
			let done = false;
			let mergedCatalogDone = false;
			try { done = !!Zotero.Prefs.get('migration.resetTranslators'); } catch (e) {}
			try {
				mergedCatalogDone = !!Zotero.Prefs.get('paperloop.migration.mergedTranslators.v1');
			}
			catch (e) {}
			if (!done || !mergedCatalogDone) {
				Zotero.debug("Migration: resetting translator caches for merged PaperLoop catalog");
				try {
				...
				Zotero.Prefs.set('paperloop.migration.mergedTranslators.v1', true);
			}
		}
