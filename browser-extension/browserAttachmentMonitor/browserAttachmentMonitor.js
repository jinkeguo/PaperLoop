...
Zotero.BrowserAttachmentMonitor = {
	...
	waitForAttachment: async function(tabId, url, timeoutMs=60000, expectedMimeType=null) {
		...
		this._addAttachmentDetectionListener(
			ruleId,
			tabId,
			resolveSucceeded,
			expectedMimeType
		...
		try {
			...
			if (Zotero.isFirefox) {
				await this._addWebRequestRule(tabId, ruleId, expectedMimeType);
			} else {
				await this._addDNRRule(tabId, ruleId, expectedMimeType);
			}
	...
	_isAttachmentResponse: function(details, expectedMimeType=null) {
		...
		contentDisposition = String(contentDisposition || '').toLowerCase();
		let hasAttachmentDisposition = contentDisposition.includes('attachment');
		let hasCorrectContentType = ['application/pdf', 'application/epub+zip'].includes(contentType);

		// When the caller expects a PDF, an HTML login/redirect page must never win merely
		// because it carries Content-Disposition: attachment. Keep listening for the real
		// binary response instead. Some publishers use octet-stream, so accept it only when
		// the URL or download filename has the expected file extension.
		if (expectedMimeType) {
			expectedMimeType = expectedMimeType.toLowerCase();
			if (contentType === expectedMimeType) return true;
			if (contentType !== 'application/octet-stream' || !hasAttachmentDisposition) {
				return false;
			}
			let extension = expectedMimeType === 'application/pdf'
				? 'pdf'
				: expectedMimeType === 'application/epub+zip' ? 'epub' : null;
			if (!extension) return false;
			let extensionPattern = new RegExp(`\\.${extension}(?:["';\\s]|$)`, 'i');
			return extensionPattern.test(contentDisposition)
				|| new RegExp(`\\.${extension}(?:[?#]|$)`, 'i').test(String(details.url || ''));
		}
		return hasAttachmentDisposition || hasCorrectContentType;
	},

	_addAttachmentDetectionListener: function(ruleId, tabId, resolveSucceeded, expectedMimeType=null) {
		let attachmentFound = false;
		const listener = (details) => {
			if (!attachmentFound && this._isAttachmentResponse(details, expectedMimeType)) {
	...
	_addWebRequestRule: async function(tabId, ruleId, expectedMimeType=null) {
		const redirectUrl = Zotero.getExtensionURL("browserAttachmentMonitor/browserAttachmentMonitor.html#success");
	
		const listener = (details) => {
			if (this._isAttachmentResponse(details, expectedMimeType)) {
				return { redirectUrl };
			}
	...
	_getDNRResponseHeaderConditions: function(expectedMimeType=null) {
		if (expectedMimeType) {
			return [{
				header: 'content-type',
				values: [`${expectedMimeType.toLowerCase()}*`]
			}];
		}
		return [
			{
	...
	_addDNRRule: async function(tabId, ruleId, expectedMimeType=null) {
		...
		await browser.declarativeNetRequest.updateSessionRules({
			...
			addRules: [{
				...
				condition: {
					...
					regexFilter: '.*',
					tabIds: [tabId],
					responseHeaders: this._getDNRResponseHeaderConditions(expectedMimeType)
				}
			}]
		});
	}
