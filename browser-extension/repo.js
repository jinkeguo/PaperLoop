...
Zotero.Repo = new function() {
	this.infoRe = /^\s*{[\S\s]*?}\s*?[\r\n]/;
	this.getCompatibilityVersion = function () {
		return ZOTERO_CONFIG.CONNECTOR_COMPATIBILITY_VERSION || Zotero.version;
	};

	this.getCodeMetadata = function (code, label) {
		var m = Zotero.Repo.infoRe.exec(code);
		if (!m) {
			throw new Error("Invalid or missing translator metadata JSON object for " + label);
		}
		try {
	...
	this.getTranslatorCode = async function (translatorID, debugMode) {
		...
		if (!ZOTERO_CONFIG.ALWAYS_FETCH_FROM_REPOSITORY) {
			...
			try {
				...
				let localMetadata = Zotero.Repo.getCodeMetadata(code, translator.label);
				if (Zotero.Date.sqlToDate(localMetadata.lastUpdated)
						< Zotero.Date.sqlToDate(translator.lastUpdated)) {
					Zotero.debug(`Repo: Zotero Client code for ${translator.label} is older than merged metadata; trying Repo`);
					code = null;
				}
			}
			catch (e) {}
		...
		if (!code && !debugMode) {
			// then try repo
			const url = `${ZOTERO_CONFIG.REPOSITORY_URL}code/${translatorID}?version=${Zotero.Repo.getCompatibilityVersion()}`;
			try {
		...
		var metadata = Zotero.Repo.getCodeMetadata(code, translator.label);
		if (Zotero.Date.sqlToDate(metadata.lastUpdated)
				< Zotero.Date.sqlToDate(translator.lastUpdated)) {
			throw new Error(`Retrieved code for ${translator.label} is older than merged translator metadata`);
		}
	...
	this.getTranslatorMetadataFromServer = async function(reset=false) {
		var url = ZOTERO_CONFIG.REPOSITORY_URL + "metadata?version=" + Zotero.Repo.getCompatibilityVersion() + "&last="+
