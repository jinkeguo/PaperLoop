...
Zotero.BotBypass.passJSDetectionViaHiddenIframe = async function(url, tab, expectedMimeType) {
	...
	const waitForAttachmentPromise = new Promise((resolve) => {
		...
		messageListener = (message) => {
			if (message.type === 'attachment-monitor-loaded' 
				...
				resolve(Zotero.BrowserAttachmentMonitor.waitForAttachment(
					tab.id,
					url,
					5000,
					expectedMimeType
...
Zotero.BotBypass.passJSDetectionViaWindowPrompt = async function(url, tab, expectedMimeType) {
